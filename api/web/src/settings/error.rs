...
impl IntoApiError for Error {
    fn into_api_error(self) -> ApiErrorResponse {
        match self {
            Error::InternalError => {
                tracing::error!("Internal error in settings API: {:?}", self);
                sentry::capture_error(&self as &dyn std::error::Error);
                errors::internal_error()
            }
            Error::NotFound => {
                errors::not_found("Setting")
            }
            Error::ServiceUnavailable => {
                errors::service_unavailable("Settings")
            }
            ...
            Error::RequestError(req_err) => {
                ...
                if req_err.is_timeout() {
                    errors::temporarily_unavailable("Settings service is temporarily unavailable")
                } else if req_err.is_connect() {
                    errors::service_unavailable("Settings")
                } else if req_err.is_status() {
                    if let Some(status) = req_err.status() {
                        match status {
                            reqwest::StatusCode::NOT_FOUND => errors::not_found("Setting"),
                            ...
                            _ => errors::service_unavailable("Settings")
                        }
                    } else {
                        errors::service_unavailable("Settings")
                    }
