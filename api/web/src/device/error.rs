...
impl IntoApiError for Error {
    fn into_api_error(self) -> ApiErrorResponse {
        match self {
            Error::InternalError => {
                tracing::error!("Internal error in device API: {:?}", self);
            ...
            Error::RequestError(req_err) => {
                ...
                if req_err.is_timeout() {
                    errors::temporarily_unavailable("Device service is temporarily unavailable")
                } else if req_err.is_connect() {
                    errors::service_unavailable("Device")
                } else if req_err.is_status() {
                    if let Some(status) = req_err.status() {
                        match status {
                            reqwest::StatusCode::NOT_FOUND => errors::not_found("Device"),
                            ...
                            _ => errors::service_unavailable("Device")
                        }
                    } else {
                        errors::service_unavailable("Device")
                    }
