...
use scripting_service::{Config, Pool};
...
fn main() {
    ...
    tokio::runtime::Builder::new_multi_thread()
        ...
        .block_on(async {
            tracing_subscriber::registry()
                .with(
                    tracing_subscriber::EnvFilter::try_from_default_env().unwrap_or_else(|_| {
                        "scripting_service=debug,tower_http=debug,axum::rejection=trace".into()
                    }),
                )
            ...
            let app = scripting_service::app(config, pool);
...
fn load_config() -> Config {
    const FILE_PATH: &str = if cfg!(debug_assertions) {
        "services/scripting_service/config/.env"
    } else {
        "config/.env"
    };
